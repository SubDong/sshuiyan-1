if (config != undefined && !config.open) {
    (function () {
        ...
        md.getDomain = function () {
            ...
            b = "." + b.replace(/:\d+/, "");
            var d = a.indexOf(b);
            return (-1 < d && d + b.length == a.length) ? b.replace(/(:\d+)?[\/\?#].*/) : "";
        };
            ...
            sta.prototype = {
                ...
                na: function () {
                    ...
                    if (null == a || undefined == a || "" == a) {
                        ...
                        if (ckDay != undefined && ckDay < newDay) {
                            this.setData("PFT_" + c.id);
                            md.g.n = "1"
                        }
                    }
                ...
                par: function () {
                    var a = "", b = h.I.Q, _c = md.g, ck = md.cookie;
                    for (var i = 0; i < b.length; i++) {
                        _c[b[i]] != undefined && _c[b[i]] != "" && _c[b[i]] != null ? a = a + b[i] + "\=" + _c[b[i]] + ((b[i] == "v") ? "" : "\&") : "";
                    }
                    ck.set("PFT_SJKD", new Date().getDate());
                    return a;
                },
                custor: function () {
                ...
                getSelectJS: function () {
                    ...
                    if (refJS.indexOf("http://" + h.I.R) === 0 || refJS.indexOf("https://" + h.I.R) === 0) {
                        var a = document.createElement("script");
                        a.setAttribute("type", "text/javascript");
                        a.setAttribute("src", h.I.protocol + "//" + h.I.RS + "/t.js/select?tid=" + c.id);
                        var f = document.getElementsByTagName("script")[0];
                        f.parentNode.insertBefore(a, f);
                    }
                },
            ...
            (function () {
                ...
                if (ck.get("PFT_DTNJ") == "true") {
                    var dur = function () {
                        ...
                        if (parseInt((dateTime - dtn) / 1000) >= config.duration.tttime) {
                            md.g.duration = 1;
                            h.b.sm();
                            md.g.duration = null;
                            clearInterval(timer);
                ...
                var timer = setInterval(dur, 1000);
            }());
        }
        if (config.visit != undefined && config.visit.pvpause) {
            ...
            (function () {
                ...
                if (ck.get("PFT_DTNP") === "true") {
                    ...
                    if (parseInt(dtn) >= config.visit.pvtimes) {
                        md.g.durPage = 1;
                        h.b.sm();
                        md.g.durPage = null;
            ...
            window.onload = function () {
                ...
                config.evt.forEach(function (item, i) {
                    ...
                    if (loc === item.evpage) {
                        ...
                        var pd = eidTree.attributes.getNamedItem("onclick");
                        console.log(pd)
                        if (eidTree != undefined && eidTree != null) {
                            if (pd == null) {
                                ...
                                if (pd.nodeValue.indexOf("_pct.putPar") == -1) {
                                    eidTree.onclick = function () {
                                        _pct.putPar(["_trackEvent", item.eid, item.evttag]);
                                    }
                                }
