...
define(["./module"], function (ctrs) {
    ...
    ctrs.controller('pagechange_update', function ($scope, $http, $rootScope, $cookieStore, $stateParams, ngDialog, $state) {
        ...
        var init = function () {
            var url = "/config/page_conv?type=search&query=" + JSON.stringify({_id: $stateParams.id});
            $http({
                method: 'GET',
                url: url
            }).success(function (dataConfig, status) {
                    if (dataConfig != null && dataConfig.length == 1) {
                        var pageConvEntity = JSON.parse(angular.toJson(dataConfig[0]))//dataConfig[0];
                        $scope.target_name = pageConvEntity.target_name;
                        $scope.target_urls = pageConvEntity.target_urls;
                        $scope.record_type = pageConvEntity.record_type;
                        $scope.chooseRecordType($scope.record_type);
                        $scope.expected_yield = pageConvEntity.expected_yield;
                        $scope.pecent_yield = pageConvEntity.pecent_yield;
                        $scope.conv_tpye = pageConvEntity.conv_tpye;
                        $scope.chooseConvType($scope.conv_tpye);
                        $scope.conv_text = pageConvEntity.conv_text;
                        $scope.paths = pageConvEntity.paths;
