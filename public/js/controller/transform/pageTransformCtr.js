...
define(["./module"], function (ctrs) {
    ...
    ctrs.controller('pageTransformCtr', function ($scope, $rootScope, $q, requestService, areaService, $http, SEM_API_URL, uiGridConstants, $cookieStore) {
            ...
            $scope.page_init = function (isContrastDataByTime) {
                ...
                $http.get("/api/transform/transformAnalysis?start=" + start + "&end=" + end + "&analysisAction=pageConversion&type=1&searchType=initAll&queryOptions=" + $scope.es_checkedArray).success(function (data) {
                        if (data != null || data != "") {
                            var hasCrate = false;
                            ...
                            if (hasCrate) {//针对转化率做特殊处理
                                ...
                                if ($scope.convert_url_all.length != 0) {
                                    ...
                                    $http({
                                        method: "GET",
                                        url: "/api/transform/transformAnalysis?start=" + start + "&end=" + end + "&analysisAction=pageConversion&type=1&searchType=queryDataByUrl&showType=total&all_urls=" + all_urls
            ...
            $scope.dataTable = function (isContrastTime, showType, queryOptions, renderLegend) {
                if (isContrastTime) {
                    ...
                    $http.get("/api/transform/transformAnalysis?start=" + $rootScope.start + "&end=" + $rootScope.end + "&analysisAction=pageConversion&type=1&searchType=contrastData&showType=" + showType + "&queryOptions=" + queryOptions + "&contrastStart=" + $scope.start + "&contrastEnd=" + $scope.end).success(function (contrastData) {
                        ...
                        if (hasSem) {
                            if (hasCrate) {
                                ...
                                if ($scope.convert_url_all.length != 0) {
                                    ...
                                    $http({
                                        method: "GET",
                                        url: "/api/transform/transformAnalysis?start=" + crate_time.start + "&end=" + crate_time.end + "&analysisAction=pageConversion&type=1&searchType=queryDataByUrl&showType=day&all_urls=" + all_urls
                                    }).success(function (all_urls_data) {
                                            var semRequest = "";
                    ...
                    $http.get("/api/transform/transformAnalysis?start=" + $rootScope.start + "&end=" + $rootScope.end + "&analysisAction=pageConversion&type=1&searchType=dataTable&showType=" + showType + "&queryOptions=" + queryOptions).success(function (data) {
                        var chart = echarts.init(document.getElementById($scope.charts[0].config.id));
                        chart.showLoading({
                            text: "正在努力的读取数据中..."
                        });
                        ...
                        if (hasSem) {
                            ...
                            if (hasCrate) {
                                ...
                                $http({
                                    method: "GET",
                                    url: "/api/transform/transformAnalysis?start=" + $rootScope.start + "&end=" + $rootScope.end + "&analysisAction=pageConversion&type=1&searchType=queryDataByUrl&showType=day&all_urls=" + all_urls
                                }).success(function (all_urls_data) {
                                    var temporaryContrastData = [];
                                    for (var i = 0; i < data.length; i++) {
